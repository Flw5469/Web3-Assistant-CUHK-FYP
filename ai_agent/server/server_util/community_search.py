...
@time_tracker
def community_search(graph, client, emb, query:str, k=5):
    ...
    for id in id_list:
        ...
        query_text = f"""match (n:Document)--()--(m:__Community__) where n.id='{id}' and m.summary is not NULL return m.id AS id, m.summary AS text limit 5"""
